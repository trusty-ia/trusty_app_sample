...
static void security_test_handle_port(const uevent_t *ev)
{
	...
	if (ev->event & IPC_HANDLE_POLL_READY) {
		...
		TLOGI("### g_test_alloc->vir_addr = 0x%p, g_test_alloc->phy_addr= 0x%p\n",
				(void*)g_test_alloc->vir_addr, (void*)g_test_alloc->phy_addr);
		...
		if(g_send_buf)
			free(g_send_buf);
		g_send_buf = calloc(1, sizeof(g_test_alloc->phy_addr)+MEM_SHOW_SIZE);
		memcpy(g_send_buf, &g_test_alloc->phy_addr, sizeof(g_test_alloc->phy_addr));
		memcpy(g_send_buf+sizeof(g_test_alloc->phy_addr), (void*)g_test_alloc->vir_addr, MEM_SHOW_SIZE);
		iov.base = g_send_buf;
		iov.len  = sizeof(g_test_alloc->phy_addr)+MEM_SHOW_SIZE;
		msg.num_iov = 1;
		msg.iov     = &iov;
		msg.num_handles = 0;
		msg.handles  = NULL;
...
static void security_test_check_handle_port(const uevent_t *ev)
{
	...
	if (ev->event & IPC_HANDLE_POLL_READY) {
		...
		TLOGI("###-check g_refer_alloc->vir_addr = 0x%p\n",
				(void*)g_refer_alloc->vir_addr);
		...
		free((void*)g_refer_alloc->vir_addr);
		free(g_refer_alloc);

		/* send heap phy addr*/
		if(g_send_buf)
			free(g_send_buf);
		g_send_buf = calloc(1, sizeof(cmp_flag)+MEM_SHOW_SIZE);
		memcpy(g_send_buf, &cmp_flag, sizeof(cmp_flag));
		memcpy(g_send_buf+sizeof(cmp_flag), (void*)g_test_alloc->vir_addr, MEM_SHOW_SIZE);
		iov.base = g_send_buf;
		iov.len  = sizeof(cmp_flag)+MEM_SHOW_SIZE;
		msg.num_iov = 1;
		msg.iov     = &iov;
		msg.num_handles = 0;
		msg.handles  = NULL;
...
static void security_test_free_handle_port(const uevent_t *ev)
{
	...
	if (ev->event & IPC_HANDLE_POLL_READY) {
		...
		// free
		free((void*)g_test_alloc->vir_addr);
		free(g_test_alloc);
		free_flag = 1;
		if(g_send_buf)
		{
